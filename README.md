...
A small **test dataset** is included in ```data``` so users can easily try main workflow without installing heavy external tools like GimmeMotifs or TOBIAS.
...
GRNforge/
├── data/
│   ├── universal_files/          # dataset-independent reusable reference files
│   └── test/                     # sample dataset (10 TFs, 100 TGs, 1 chromosome)
...
- RNA-seq & ATAC-seq preprocessing
    - Expected output from the user is AnnData objects (`h5ad` files), which contain cell and gene information for each activity, with normalized value (gene activity from ATAC and gene expresison from RNA). 
- TFBS detection: to detect TF binding sites, we use
    ...
    ```
To simplify the test run, we provide a dataset that includes gene activity and gene expression (see description in [section 8](#8-test-dataset)) and output files of GimmeMotifs (TF-motif connection) and TOBIAS (TFBS-RE connection), so you may skip the preprocesisng step and step using  GimmeMotifs and tobias. You only need to run the python file to gain the TFBS_peak file needed as input to XGBoost.
...
The network inference consists of (1) base-network construction and (2) predictive modelling with XGBoost.   
- Base eGRN construction `baseGRN.py`: This step builds RE–TG links (distance-based), TF–RE links (motif + footprint), TF–TG candidate interactions (see figure above)
...
After running predictive modelling with XGBoost, GRN postprocessing follows. This step:

- normalizes raw importance scores
- integrates results across k-folds and models
- ranks final TF–TG interactions
- aggregates all outputs into final eGRN files and calculates TF activities for all models (integrated and not)
...
Before starting the GRNforge pipeline, please download your preferred gene annotation ([section 7](#7-additional-data)) and store it in `data/universal_files`
...
## 7. Additional data
Gene annotation (e.g. GENCODE v38)
```
...
## 8. Test dataset
Test dataset is a subset of the [10k PBMC Multiome dataset](https://www.10xgenomics.com/datasets/10-k-human-pbm-cs-multiome-v-1-0-chromium-x-1-standard-2-0-0), containing chromatin from chromosome 4 only, and expression of 100 target genes (of which 10 are TFs). Do not forget to download and store the gene annotation.   
The folder `data/test` contains:
- processed RNA expression matrices
- processed ATAC gene activity matrices
- precomputed peak-motif pairs
- precomputed footprint scores

This allows running the entire workflow without either needing preprocessing or using GimmeMotifs or TOBIAS.

## 9. Citation

If you use our work, please cite GRNforge by clicking "Cite this repository" in the side bar of the repo and choose your preferred option.   The button is right below the license, you can also find it by ctrl+F.
